...
def category(id):
    ...
    while not categories:
        try:
            ...
            print("CardCategory.query.all() run fail")
            categories=[]
        time.sleep(0.2)
    all_cards = []
    for category in categories:
        now_cards = category.cards
        all_cards.extend(now_cards)

    if id == 2:
        category = CardCategory.query.get_or_404(2)
    ...
    for card in all_undone_cards:
        ...
        if leave_day > 1 and leave_day <= 7:
            week_cards.append(card)
        if leave_day > 7 and leave_day <= 30:
            month_cards.append(card)
        if leave_day > 30 and leave_day <= 365:
    ...
    category = CardCategory.query.get_or_404(id)
    items = category.cards
...
def new_category():
    ...
    category = CardCategory(name=name)
...
def edit_card(id):
    ...
    card_0 = Card.query.get_or_404(id)
